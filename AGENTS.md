...
**DO NOT modify existing Modular kernel code.** Import existing kernels if needed. All new code goes in the `examples/` directory.
...
| Path | Description |
|------|-------------|
| `examples/custom_ops/kernels/` | Mojo kernel implementations |
| `examples/custom-models/gpt_oss_mxfp4/` | GPT-OSS model architecture and Python bindings |
| `examples/custom-models/triton_example/` | Reference Triton implementation (kernel pattern to follow) |
...
3. **Review the Triton Reference** — `examples/custom-models/triton_example/`
...
3. **Kernel pattern:** Study `examples/custom-models/triton_example/` for working SM90 code
4. **Write kernels:** Add Mojo kernels in `examples/custom_ops/kernels/`
5. **Model integration:** Update architecture in `examples/custom-models/gpt_oss_mxfp4/`
