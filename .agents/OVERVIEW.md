...
* Hardware fact: SM90 has tensor cores for **BF16/FP16/FP8/INT8**, but **no native FP4**. So, just like the OpenAI Triton code, the only sensible play is:
  ...
  * **Dequantize to FP16 inside the GEMM**,
  * Feed FP16 fragments into **wgmma**.

We’re going to design that GEMM, with:

* A: BF16 activations.
* B: MXFP4 weights (uint8 + float8 scales).
* C: BF16 output (with FP32 accumulation).
...
* Shapes:
  ...
  * `B`: `[K, N]` in **logical** BF16, but **stored as MXFP4 blocks**:
    ...
    * Each 32-wide group has a **scale** in `float8_e8m0fnu` (your `MXFP4_SF_DTYPE`).
  * `C`: `[M, N]` BF16.
...
* Kernel mapping:
  ...
  * For each chunk:
    ...
    * Each warp:
      ...
      * Runs `wgmma` (FP16 × FP16 → FP32 accum).
