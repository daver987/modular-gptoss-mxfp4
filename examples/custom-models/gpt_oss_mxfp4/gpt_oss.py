...
class GptOssTextModel(Module):
    ...
    def __init__(self, config: GptOssConfig) -> None:
        ...
        self.norm.sharding_strategy = ShardingStrategy.replicate(len(config.devices))
        ...
        self.lm_head = ColumnParallelLinear(
            ...
            tied_weight=(self.embed_tokens.weight if config.tie_word_embeddings else None),
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for idx, layer in enumerate(self.layers):
            layer_idx_tensor = ops.constant(idx, DType.uint32, device=self.devices[0])
            h = layer(
                layer_idx_tensor,
                h,
                signal_buffers,
        ...
        if h:
            last_token_h = [
                ops.gather(h_device, indices, axis=0) for h_device, indices in zip(h, last_token_indices, strict=True)
            ]
        last_logits = ops.cast(
            # Take only the device 0 logits to device-to-host transfer.
            self.lm_head(
                [self.norm_shards[i](last_token_h[i]) for i in range(len(last_token_h))],
                signal_buffers,
            )[0],
            DType.float32,
        )
...
class GptOssMXFP4(Module):
    """The GPT OSS model."""

    def __init__(self, config: GptOssMXFP4Config) -> None:
        super().__init__()
